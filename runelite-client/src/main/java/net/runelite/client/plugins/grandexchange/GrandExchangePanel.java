...
class GrandExchangePanel extends PluginPanel
{
	...
	void updateOffers()
	{
		if (client.getGameState() != GameState.LOGGED_IN || offers == null)
		{
			return;
		}
		...
		for (GrandExchangeOfferSlot slot : offers)
		{
			...
			EventQueue.invokeLater(() ->
			{
				try
				{
					...
					slot.setOfferImage(itemId == 0 ? null : new ImageIcon(itemImage));
					String stackSize = ItemManager.quantityToStackSize(price);
					slot.setOfferPriceLabel(price == 0 ? "" :
							stackSize.endsWith("M") || stackSize.endsWith("K")
									? stackSize
									: stackSize + " gp");
					slot.getProgressBar().setString(total == 0 ? "" : sold + " / " + total);
	...
	Consumer<GrandExchangeOfferChanged> updateConsumer = (incoming) ->
	{
		offers[incoming.getIndex()].offer = incoming.getOffer();
		updateOffers();
	};
