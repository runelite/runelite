...
public class KittenNotifierPlugin extends Plugin{
    ...
    public void onNpcSpawned(NpcSpawned event) {
        if (event.getNpc().getName().contentEquals("Kitten")) {
            if (event.getNpc().getInteracting().getName().contentEquals(client.getLocalPlayer().getName())) {
                config.catOwned(false);
            }
        }
        else if (event.getNpc().getName().contentEquals("Cat")) {
            if (event.getNpc().getInteracting().getName().contentEquals(client.getLocalPlayer().getName())) {
                config.catOwned(true);
            }
        }
