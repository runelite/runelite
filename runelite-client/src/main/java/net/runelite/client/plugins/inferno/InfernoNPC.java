...
class InfernoNPC
{
    ...
    void gameTick(Client client, WorldPoint lastPlayerLocation, boolean finalPhase)
    {
        ...
        if (ticksTillNextAttack <= 0)
        {
            ...
            {
                case ZUK:
                    if (this.getNpc().getAnimation() == CustomAnimationID.TZKAL_ZUK)
                    {
                        if (finalPhase)
                        {
                ...
                case BAT:
                    ...
                    if (this.canAttack(client, client.getLocalPlayer().getWorldLocation())
                            && this.getNpc().getAnimation() != CustomAnimationID.JAL_MEJRAH_STAND && this.getNpc().getAnimation() != -1)
                    {
                        this.updateNextAttack(this.getType().getDefaultAttack(), this.getType().getTicksAfterAnimation());
                    }
                    break;
                ...
                case MAGE:
                    ...
                    if (this.getNpc().getAnimation() == CustomAnimationID.JAL_IMKOT
                            || this.getNpc().getAnimation() == CustomAnimationID.JAL_XIL_RANGE_ATTACK || this.getNpc().getAnimation() == CustomAnimationID.JAL_XIL_MELEE_ATTACK
                            || this.getNpc().getAnimation() == CustomAnimationID.JAL_ZEK_MAGE_ATTACK || this.getNpc().getAnimation() == CustomAnimationID.JAL_ZEK_MELEE_ATTACK)
                    {
                        this.updateNextAttack(this.getType().getDefaultAttack(), this.getType().getTicksAfterAnimation());
                    }
    ...
    enum Attack
    {
        MELEE(Prayer.PROTECT_FROM_MELEE,
                ...
                new int[]{
                        ...
                        CustomAnimationID.JAL_NIB,
                        CustomAnimationID.JAL_AK_MELEE_ATTACK,
                        CustomAnimationID.JAL_IMKOT,
                        CustomAnimationID.JAL_XIL_MELEE_ATTACK,
                        CustomAnimationID.JAL_ZEK_MELEE_ATTACK, //TODO: Yt-HurKot attack animation
                }),
        RANGED(Prayer.PROTECT_FROM_MISSILES,
                Color.GREEN,
                new Color(0, 128, 0),
                new int[]{
                        CustomAnimationID.JAL_MEJRAH,
                        CustomAnimationID.JAL_AK_RANGE_ATTACK,
                        CustomAnimationID.JAL_XIL_RANGE_ATTACK,
                        CustomAnimationID.JALTOK_JAD_RANGE_ATTACK,
                }),
        MAGIC(Prayer.PROTECT_FROM_MAGIC,
                Color.CYAN,
                Color.BLUE,
                new int[]{
                        CustomAnimationID.JAL_AK_MAGIC_ATTACK,
                        CustomAnimationID.JAL_ZEK_MAGE_ATTACK,
                        CustomAnimationID.JALTOK_JAD_MAGE_ATTACK
                }),
