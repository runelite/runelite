...
final class IntegerToken implements class0 {
   @ObfuscatedName("x")
   @ObfuscatedSignature(
      signature = "Lid;"
   )
      ...
      for(var2 = 0; var2 < Client.pendingNpcFlagsCount; ++var2) {
         ...
         if((var5 & 4) != 0) {
            ...
            if(var6 == var4.animation && var6 != -1) {
               var8 = GrandExchangeEvents.getAnimation(var6).replyMode;
               if(var8 == 1) {
                  var4.actionFrame = 0;
                  var4.field1180 = 0;
            ...
            } else if(var6 == -1 || var4.animation == -1 || GrandExchangeEvents.getAnimation(var6).forcedPriority >= GrandExchangeEvents.getAnimation(var4.animation).forcedPriority) {
               var4.animation = var6;
               var4.actionFrame = 0;
               var4.field1180 = 0;
               var4.actionAnimationDisable = var7;
   ...
   static boolean isFriended(String var0, boolean var1) {
      if(var0 == null) {
         ...
         for(int var3 = 0; var3 < Client.friendCount; ++var3) {
            if(var2.equalsIgnoreCase(GraphicsObject.cleanUsername(Client.friends[var3].name, class236.jagexLoginType)) && (!var1 || Client.friends[var3].world != 0)) {
               return true;
            }
         }

         if(var2.equalsIgnoreCase(GraphicsObject.cleanUsername(class181.localPlayer.name, class236.jagexLoginType))) {
            return true;
         } else {
            return false;
         }
