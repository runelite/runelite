import net.runelite.mapping.Export;
import net.runelite.mapping.Implements;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("ev")
@Implements("Frame")
public class Frame {
   @ObfuscatedName("j")
   static int[] field1895;
   @ObfuscatedName("h")
   static int[] field1891;
   @ObfuscatedName("f")
   static int[] field1890;
   @ObfuscatedName("p")
   static int[] field1893;
   @ObfuscatedName("o")
   @Export("showing")
   boolean showing;
   @ObfuscatedName("x")
   @ObfuscatedSignature(
      signature = "Lel;"
   )
   @Export("skin")
   FrameMap skin;
   @ObfuscatedName("g")
   int field1894;
   @ObfuscatedName("c")
   int[] field1896;
   @ObfuscatedName("l")
   @Export("translator_x")
   int[] translator_x;
   @ObfuscatedName("w")
   @Export("translator_y")
   int[] translator_y;
   @ObfuscatedName("b")
   @Export("translator_z")
   int[] translator_z;

   static {
      field1895 = new int[500];
      field1891 = new int[500];
      field1890 = new int[500];
      field1893 = new int[500];
   }

   @ObfuscatedSignature(
      signature = "([BLel;)V"
   )
   Frame(byte[] var1, FrameMap var2) {
      this.skin = null;
      this.field1894 = -1;
      this.showing = false;
      this.skin = var2;
      Buffer var3 = new Buffer(var1);
      Buffer var4 = new Buffer(var1);
      var3.offset = 2;
      int var5 = var3.readUnsignedByte();
      int var6 = -1;
      int var7 = 0;
      var4.offset = var5 + var3.offset;

      int var8;
      for(var8 = 0; var8 < var5; ++var8) {
         int var9 = var3.readUnsignedByte();
         if(var9 > 0) {
            if(this.skin.types[var8] != 0) {
               for(int var10 = var8 - 1; var10 > var6; --var10) {
                  if(this.skin.types[var10] == 0) {
                     field1895[var7] = var10;
                     field1891[var7] = 0;
                     field1890[var7] = 0;
                     field1893[var7] = 0;
                     ++var7;
                     break;
                  }
               }
            }

            field1895[var7] = var8;
            short var11 = 0;
            if(this.skin.types[var8] == 3) {
               var11 = 128;
            }

            if((var9 & 1) != 0) {
               field1891[var7] = var4.readShortSmart();
            } else {
               field1891[var7] = var11;
            }

            if((var9 & 2) != 0) {
               field1890[var7] = var4.readShortSmart();
            } else {
               field1890[var7] = var11;
            }

            if((var9 & 4) != 0) {
               field1893[var7] = var4.readShortSmart();
            } else {
               field1893[var7] = var11;
            }

            var6 = var8;
            ++var7;
            if(this.skin.types[var8] == 5) {
               this.showing = true;
            }
         }
      }

      if(var1.length != var4.offset) {
         throw new RuntimeException();
      } else {
         this.field1894 = var7;
         this.field1896 = new int[var7];
         this.translator_x = new int[var7];
         this.translator_y = new int[var7];
         this.translator_z = new int[var7];

         for(var8 = 0; var8 < var7; ++var8) {
            this.field1896[var8] = field1895[var8];
            this.translator_x[var8] = field1891[var8];
            this.translator_y[var8] = field1890[var8];
            this.translator_z[var8] = field1893[var8];
         }

      }
   }
}
