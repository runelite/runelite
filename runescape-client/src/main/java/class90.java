import java.math.BigInteger;
import net.runelite.mapping.Export;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("cc")
public class class90 {
   @ObfuscatedName("w")
   @Export("rsaKeyModulus")
   static final BigInteger rsaKeyModulus;
   @ObfuscatedName("i")
   @Export("rsaKeyExponent")
   static final BigInteger rsaKeyExponent;

   static {
      rsaKeyExponent = new BigInteger("10001", 16);
      rsaKeyModulus = new BigInteger("", 16);
   }

   @ObfuscatedName("w")
   @ObfuscatedSignature(
      signature = "(LPacketBuffer;I)V",
      garbageValue = "-1689035948"
   )
   static final void method1722(PacketBuffer var0) {
      int var1 = 0;
      var0.method3410();

      int var2;
      int var3;
      int var4;
      for(var2 = 0; var2 < class96.field1521; ++var2) {
         var3 = class96.field1531[var2];
         if((class96.field1516[var3] & 1) == 0) {
            if(var1 > 0) {
               --var1;
               class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
            } else {
               var4 = var0.method3388(1);
               if(var4 == 0) {
                  var1 = class165.method3070(var0);
                  class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
               } else {
                  class34.method505(var0, var3);
               }
            }
         }
      }

      var0.method3384();
      if(var1 != 0) {
         throw new RuntimeException();
      } else {
         var0.method3410();

         for(var2 = 0; var2 < class96.field1521; ++var2) {
            var3 = class96.field1531[var2];
            if((class96.field1516[var3] & 1) != 0) {
               if(var1 > 0) {
                  --var1;
                  class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
               } else {
                  var4 = var0.method3388(1);
                  if(var4 == 0) {
                     var1 = class165.method3070(var0);
                     class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
                  } else {
                     class34.method505(var0, var3);
                  }
               }
            }
         }

         var0.method3384();
         if(var1 != 0) {
            throw new RuntimeException();
         } else {
            var0.method3410();

            for(var2 = 0; var2 < class96.field1523; ++var2) {
               var3 = class96.field1524[var2];
               if((class96.field1516[var3] & 1) != 0) {
                  if(var1 > 0) {
                     --var1;
                     class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
                  } else {
                     var4 = var0.method3388(1);
                     if(var4 == 0) {
                        var1 = class165.method3070(var0);
                        class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
                     } else if(class149.method2899(var0, var3)) {
                        class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
                     }
                  }
               }
            }

            var0.method3384();
            if(var1 != 0) {
               throw new RuntimeException();
            } else {
               var0.method3410();

               for(var2 = 0; var2 < class96.field1523; ++var2) {
                  var3 = class96.field1524[var2];
                  if((class96.field1516[var3] & 1) == 0) {
                     if(var1 > 0) {
                        --var1;
                        class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
                     } else {
                        var4 = var0.method3388(1);
                        if(var4 == 0) {
                           var1 = class165.method3070(var0);
                           class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
                        } else if(class149.method2899(var0, var3)) {
                           class96.field1516[var3] = (byte)(class96.field1516[var3] | 2);
                        }
                     }
                  }
               }

               var0.method3384();
               if(var1 != 0) {
                  throw new RuntimeException();
               } else {
                  class96.field1521 = 0;
                  class96.field1523 = 0;

                  for(var2 = 1; var2 < 2048; ++var2) {
                     class96.field1516[var2] = (byte)(class96.field1516[var2] >> 1);
                     Player var5 = Client.cachedPlayers[var2];
                     if(var5 != null) {
                        class96.field1531[++class96.field1521 - 1] = var2;
                     } else {
                        class96.field1524[++class96.field1523 - 1] = var2;
                     }
                  }

               }
            }
         }
      }
   }
}
