import java.math.BigInteger;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName("af")
public class class36 {
   @ObfuscatedName("n")
   static final BigInteger field765 = new BigInteger("7237300117305667488707183861728052766358166655052137727439795191253340127955075499635575104901523446809299097934591732635674173519120047404024393881551683");
   @ObfuscatedName("y")
   static String field767;
   @ObfuscatedName("g")
   static final BigInteger field769 = new BigInteger("80782894952180643741752986186714059433953886149239752893425047584684715842049");
   @ObfuscatedName("du")
   @ObfuscatedGetter(
      intValue = 1331176933
   )
   static int field777;

   @ObfuscatedName("q")
   @ObfuscatedSignature(
      signature = "(I)Z",
      garbageValue = "1305782182"
   )
   public static boolean method696() {
      return class138.field1914 != 0?true:class138.field1913.method2460();
   }
}
